...
function AllocationRow({ allocation }) {
  ...
  const allocId = allocation.id ?? allocation._id;
  async function handleDelete() {
    try {
      await remove.mutateAsync(allocId);
  ...
  return (
    <li className="rounded-[10px] border border-[#E2E8F0] p-4">
      ...
      {/* Fulfil this order with trees that are already planted on the site
          (e.g. bulk-imported historical trees) instead of planting new ones. */}
      <div className="mt-3 border-t border-[#E2E8F0] pt-3">
        ...
        >
          ...
          Assign existing trees
          <ChevronDown className={cn('h-3.5 w-3.5 transition-transform', showAssign && 'rotate-180')} aria-hidden />
        </button>
        {showAssign && <AttachTreesPanel allocationId={allocId} onDone={() => setShowAssign(false)} />}
      </div>
    </li>
  );
}
